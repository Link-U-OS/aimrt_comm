...
{
class CounterContextTest : public aimrte::test::TestBase
{
 protected:
...
TEST_F(CounterContextTest, CounterDefault)
{
  ctrl.LetInit();
  ...
  EXPECT_CALL(pub_mocker, Analyze).WillRepeatedly([&is_called](const aimrte::trace::internal::EventChannel& msg) {
    ...
    GTEST_ASSERT_EQ(event.attributes.at("type"), "Counter");
    GTEST_ASSERT_EQ(event.status, aimrte::trace::internal::Event::Status::SUCCESS);
    GTEST_ASSERT_EQ(event.attributes.at("value"), "1");
    is_called = true;
  });

  aimrte::trace::Counter<EventCode::TestCounter>();
  EXPECT_TRUE(is_called);
}

TEST_F(CounterContextTest, Counter)
{
  ctrl.LetInit();
  auto& trace_ctx = ctrl.GetContext().InitSubContext<aimrte::trace::internal::Context>(aimrte::core::Context::SubContext::Trace);
  ...
  EXPECT_CALL(pub_mocker, Analyze).WillRepeatedly([&is_called](const aimrte::trace::internal::EventChannel& msg) {
    ...
    GTEST_ASSERT_EQ(event.attributes.at("type"), "Counter");
    GTEST_ASSERT_EQ(event.status, aimrte::trace::internal::Event::Status::SUCCESS);
    GTEST_ASSERT_EQ(event.attributes.at("value"), "4");
    is_called = true;
  });

  aimrte::trace::Counter<EventCode::TestCounter>(4);
  EXPECT_TRUE(is_called);
}
